...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (usePagination) {
      ...
      const pageSize = Math.min(1000, Math.max(1, Number(pageSizeParam) || 50));

      const [countResult, customersResult, accountsResult] = await Promise.all([
      ...
      return NextResponse.json(
        { customers: data, totalCount, page, pageSize },
