    ...
    return NextResponse.json(
      {
        rows: (rows ?? []).map((r: Record<string, unknown>) => ({
          ...
          originMonth: r.origin_month != null ? Number(r.origin_month) : null,
          originYear: r.origin_year != null ? Number(r.origin_year) : null,
        })),
        summary: {
          monthlyReplacement: Number(metricsRow?.monthly_replacement ?? 0),
          finishedCards: Number(metricsRow?.finished_cards ?? 0),
          currentMonthlyCollection: Number(metricsRow?.current_monthly_collection ?? 0),
